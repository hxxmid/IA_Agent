...
if uploaded_file:
    ...
    en_tetes = ["Fichier", "Page", "Employeur", "Matricule", "Nom", "Prénom", "Adresse", "Date entrée", "Emploi",
                "Catégorie", "Paiement le", "Horaire", "Salaire", "Num Sécurité Sociale"]
    ...
    def extraire_infos(page_text):
        ...
        if match_nom:
            ...
            # Cas 1 : Titre + NOM (maj) + Prénom (après)
            match_nom = re.search(
                r'\b(Mme|M|Mr|Madame|Monsieur)\s+([A-ZÉÈÀÂÙÎÏÔÇ\-]{2,})\s+([A-Za-zéèêëîïôöüàâùûç\'\- ]+)', full_text)
            if match_nom:
                infos["Nom"] = match_nom.group(2).strip()
                infos["Prénom"] = match_nom.group(3).strip()
            else:
                # Cas 2 : Prénom (min) + "Monsieur" + NOM (maj)
                match_nom2 = re.search(
                    r'\b([A-Za-zéèêëîïôöüàâùûç\'\- ]+)\s+(Monsieur|Madame|Mme|M|Mr)\s+([A-ZÉÈÀÂÙÎÏÔÇ\-]{2,})\b',
                    full_text)
                if match_nom2:
        ...
        for idx, line in enumerate(lines):
            ...
            if infos["Horaire"] == "":
                ...
                match_horaire = re.search(
                    r'(Horaire|Heures|Nb heures|Nombre d\'heures)[^\d]{0,20}(1[0-9]{2}\.[0-9]{2})', line, re.IGNORECASE)
                if match_horaire:
                    ...
                    for k in range(1, 3):
                        if idx + k < len(lines):
                            match_horaire_next = re.search(
                                r'(Horaire|Heures|Nb heures|Nombre d\'heures)[^\d]{0,20}(1[0-9]{2}\.[0-9]{2})',
                                lines[idx + k], re.IGNORECASE)
                            if match_horaire_next:
                                infos["Horaire"] = match_horaire_next.group(2)
                                break
                # Si toujours rien, cherche dans tout le texte de la page
                if infos["Horaire"] == "":
                    match_horaire_full = re.search(
                        r'(Horaire|Heures|Nb heures|Nombre d\'heures)[^\d]{0,20}(1[0-9]{2}\.[0-9]{2})', full_text,
                        re.IGNORECASE)
            ...
            match_paiement = re.search(
                r'(Paiement le|Période|Période du)\s*[:\-]?\s*([0-9]{1,2}/[0-9]{1,2}/[0-9]{2,4})', line, re.IGNORECASE)
            if match_paiement:
            ...
            for k in range(3):
                if idx + k < len(lines):
                    l = lines[idx + k]
                    match_categorie = re.search(r'(Statut professionnel|catégorie)\s*[:\-]?\s*(.*?)(?:\s+Horaire|$)', l,
                                                re.IGNORECASE)
                    if match_categorie:
                        break
            if match_categorie:
                    ...
                    if idx + k < len(lines):
                        if "Emploi" in lines[idx]:
                            ...
                            if candidate and not re.search(
                                    r'(Matricule|Entrée|Date|Salaire|Niveau|Employeur|Adresse|Séc|SS|Emploi)',
            ...
            if infos["Salaire"] == "":
                for mot_cle in ["Salaire", "Net", "Payé", "Paye", "Total", "brut", "imposable"]:
                    ...
                    if matches:
                        try:
                            valeurs = [float(m.replace(' ', '').replace('\u202f', '').replace(',', '.')) for m in
                                       matches]
            ...
            if index_emp != -1:
                for j in range(index_emp + 1, index_emp + 4):
                    if j < len(lines):
                        ...
                        if re.search(r'\d{2,}.*(RUE|AVENUE|BD|BOULEVARD|IMPASSE|PLACE|ALLÉE|CHEMIN|ROUTE)', candidate,
                                     re.IGNORECASE):
    ...
    st.download_button("📥 Télécharger en JSON", data=df.to_json(orient="records", force_ascii=False),
                       file_name="infos_bulletin.json")
